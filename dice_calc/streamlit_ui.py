...
def run_web():
    """Launch the Streamlit web UI via the ``trpg-web`` console script."""
    import sys
...
# Wrapped in a function so the module structure is explicit.
# Called at module level — required for ``streamlit run`` to work.
# When ``run_web()`` is used (e.g. via the ``trpg-web`` console script),
# :func:`streamlit.web.cli.main` re-launches a fresh process where this
# function executes with a proper Streamlit context.
...
# Render the app at module level — required for ``streamlit run`` to work.
_render_app()
